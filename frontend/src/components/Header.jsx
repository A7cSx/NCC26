...
import { Languages, LogOut, Trophy, ShieldCheck } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 glass border-b border-white/5" data-testid="app-header">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        <Link to="/" className="flex items-center gap-3 group" data-testid="logo-link">
          <div className="w-10 h-10 rounded-lg bg-white/95 p-1 flex items-center justify-center shadow-lg shadow-ncc-teal/20 group-hover:shadow-ncc-teal/40 transition-shadow">
          ...
          {user ? (
            <div className="flex items-center gap-2">
              <div className="hidden sm:block text-right">
                ...
                <div className="text-sm font-bold truncate max-w-[120px]" data-testid="user-name">{user.name}</div>
              </div>
              <button
